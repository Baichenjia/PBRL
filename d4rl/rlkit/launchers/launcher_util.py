...
def create_exp_name(exp_prefix, exp_id=0, seed=0):
    ...
    return "seed_%d-%s" % (seed, timestamp)
...
def setup_logger(
    ...
    if first_time:   # 创建文件夹
        log_dir = create_log_dir(exp_prefix, seed=variant['seed'], **create_log_dir_kwargs)

    if variant is not None:
        logger.log("Variant:")
        logger.log(json.dumps(dict_to_safe_json(variant), indent=2))  # json 文件
