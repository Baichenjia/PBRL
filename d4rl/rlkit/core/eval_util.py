...
def get_generic_path_information(paths, stat_prefix=''):
    """
    函数在每个epoch (1000 steps) 的训练结束后被调用, 来返回 evaluation paths 里面的 return 信息
    Get an OrderedDict with a bunch of statistic names and values.
    """
    statistics = OrderedDict()
    # 每个周期的 return
    returns = [sum(path["rewards"]) for path in paths]
    # 每个周期的每个时间步的 steps. 组成一个 list
    rewards = np.vstack([path["rewards"] for path in paths])
    # 计算 rewards list 的 mean/max/min/std 等统计量
    statistics.update(create_stats_ordered_dict('Rewards', rewards, stat_prefix=stat_prefix))
    # 计算 return list 的 mean/max/min/std 等统计量
    statistics.update(create_stats_ordered_dict('Returns', returns, stat_prefix=stat_prefix))
    # 计算 action list 的 mean/max/min/std 等统计量
    ...
    statistics['Num Paths'] = len(paths)    # evaluation 的总周期数目
    statistics[stat_prefix + 'Average Returns'] = get_average_returns(paths)   # 周期的平均奖励
