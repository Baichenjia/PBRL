...
class TanhGaussianPolicy(Mlp, ExplorationPolicy):
    ...
    def __init__(self, hidden_sizes, obs_dim, action_dim, std=None, init_w=1e-3, **kwargs):
        # hidden_sizes=[256, 256, 256], obs_dim=17, action_dim=6, std=None
    ...
    def log_prob(self, obs, actions):
        ...
        mean = self.last_fc(h)
        mean = torch.clamp(mean, MEAN_MIN, MEAN_MAX)   # -9.0, 9.0

        if self.std is None:  
            log_std = self.last_fc_log_std(h)                           # log_std
            log_std = torch.clamp(log_std, LOG_SIG_MIN, LOG_SIG_MAX)    # [-5, 2]
    ...
    def forward(self, obs, reparameterize=True, deterministic=False, return_log_prob=False):
        ...
        return (action, mean, log_std, log_prob, entropy, std,  
