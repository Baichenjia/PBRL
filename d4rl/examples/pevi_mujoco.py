...
def experiment(variant, log_dir=None):
    ...
    eval_policy = MakeDeterministic(policy)   # for evaluation
    eval_path_collector = MdpPathCollector(eval_env, eval_policy)
    expl_path_collector = CustomMDPPathCollector(eval_env)

    # Replay Buffer. load offline data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ucb_ratio', default=0.01, type=float)
    parser.add_argument('--ensemble', default=10, type=int)
    parser.add_argument('--min_weight_ood', default=0.2, type=float)
    parser.add_argument('--decay_factor', default=1.01, type=float)
    ...
    variant['trainer_kwargs']['min_weight_ood'] = args.min_weight_ood
    ...
    logdir = setup_logger("", variant=variant, base_log_dir=os.path.join("result-PEVI-finally",
